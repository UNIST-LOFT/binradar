...
def test_filter_meta_identity(tmp_path):
    """filter.sbsv metadata pins kind + predicates SHA-256."""
    load = binradar_setup.load_filter_passed_ids
    write = binradar_setup.write_filter
    sha = binradar_setup.predicates_sha256

    predicates = tmp_path / "predicates"
    predicates.write_text("max1 - rdx == ~max1\n")
    sbsv = tmp_path / "filter.sbsv"
    ...
    missing_meta = tmp_path / "missing-meta.sbsv"
    missing_meta.write_text(
        "[filter] [res] [id 1] [pass true] [new-id 1]\n"
        "[filter] [done] [total 1] [survived 1] [time 0.01]\n")
    assert load(missing_meta, expected_kind="generic-erm",
                expected_sha256=sha(predicates)) is None
    assert load(missing_meta) == {1: 1}
    # The meta row is written first.
    assert sbsv.read_text().startswith(
        f"[filter] [meta] [version 1] [kind generic-erm] [sha256 {sha(predicates)}]\n")
