...
void snapshot_load_inferred_types(FILE *fp) {
    sbsv_parser *parser = sbsv_parser_new(1);
    ...
    sbsv_parser_add_schema(parser, "[type-def] [primitive] [id: str] [sz: int] [body: str]");
    ...
    for (size_t i = 0; i < num_rows; i++) {
        ...
        int size = sbsv_row_get_int(r, "size", &valid);
        const char *body = sbsv_row_get_string(r, "body");
        trace_mem("[test-type-def] [id %s] [size %d] [body %s]\n", id, size, body);
    }
    sbsv_free_row_ref_array(rows);
    ...
    for (size_t i = 0; i < num_rows; i++) {
        ...
        uint64_t off = sbsv_row_get_int(r, "off", &valid);
        uint64_t size = sbsv_row_get_int(r, "size", &valid);
        uint64_t target = sbsv_row_get_int(r, "target", &valid);
        const char *type_id = sbsv_row_get_string(r, "type-id");
        double p = sbsv_row_get_float(r, "P", &valid);
        trace_mem("[test-pointer] [region-type %d] [region-base %lx] [region-size %lx] [off %lx] [size %lx] [target %lx] [type-id %s] [P %f]\n", region_type, region_base, region_size, off, size, target, type_id, p);
    }
    sbsv_free_row_ref_array(rows);
    sbsv_parser_free(parser);
}
...
int main(int argc, char* argv[]) {
    ...
    if (input_fp != NULL) {
        snapshot_load_inferred_types(input_fp);
        fclose(input_fp);
    }
