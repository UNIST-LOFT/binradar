...
#define OPERATION(a, b, size, operator, res)                                    \
    ...
    static uint64_t Z3_custom_eval_internal_iter(Z3_context c, Z3_ast _expr, uint64_t* data, uint8_t* symbols_sizes, size_t data_size) {
        ...
        uint64_t res = 0; // hold the result from last iteration
        ...
        while (!m_frame_stack.empty()) {
            ...
            if (f.m_num_args == 0) {
                ...
                if (f.m_fid == 0x6) {
                    switch(f.m_decl_kind) {
                        case OP_BNEG: {
                            res = (-res) & MASK(SIZE(APP(f.m_curr)));
                            break;
                        }
                        case OP_BNOT: {
                            res = (~res) & MASK(SIZE(APP(f.m_curr)));
                        ...
                        case OP_ROTATE_LEFT: {
                            unsigned amount =
                                f.m_info->get_parameters()[0].get_int();
                            switch (SIZE(APP(f.m_curr))) {
                            case 8: res = __rolb(res, amount); break;
                            case 16: res = __rolw(res, amount); break;
                            case 32: res = __rold(res, amount); break;
                            case 64: res = __rolq(res, amount); break;
                            default: ERROR("unexpected size");
                            }
                            break;
                        }
                        case OP_ROTATE_RIGHT: {
                            unsigned amount =
                                f.m_info->get_parameters()[0].get_int();
                            switch (SIZE(APP(f.m_curr))) {
                            case 8: res = __rorb(res, amount); break;
                            case 16: res = __rorw(res, amount); break;
                            case 32: res = __rord(res, amount); break;
                            case 64: res = __rorq(res, amount); break;
                            default: ERROR("unexpected size");
                            }
                            break;
                        }
                        ...
                        default:
                            if (f.m_num_args == 1) {
                                ERROR("Unknown unary BV operator");
                            }
                            break;
                    }
                } else { // f.m_fid == 0
                    switch(f.m_decl_kind) {
                        ...
                        default:
                            if (f.m_num_args == 1) {
                                ERROR("Unknown unary BASE operator");
                            }
                            break;
                    }
                }
                ...
                if (f.m_fid == 0x6) {
                    switch(f.m_decl_kind) {
                        ...
                        case OP_BSDIV: {
                            if (res == 0) {
                                unsigned width = SIZE(APP(f.m_curr));
                                res = (f.m_res & (1ULL << (width - 1)))
                                    ? 1 : MASK(width);
                            } else {
                        ...
                        case OP_BSREM: {
                            if (res == 0) {
                                res = f.m_res & MASK(SIZE(APP(f.m_curr)));
                            } else {
                                OPERATION(f.m_res, res, SIZE(APP(f.m_curr)), %, res);
                            }
                            break;
                        ...
                        case OP_BSDIV_I: {
                            if (res == 0) {
                                ...
                                OPERATION(f.m_res, res,
                                          SIZE(APP(f.m_curr)), /, res);
                            }
                            break;
                        }
                        case OP_BUDIV_I: {
                            res = res == 0 ? MASK(SIZE(APP(f.m_curr))) :
                                (f.m_res / res) & MASK(SIZE(APP(f.m_curr)));
                            break;
                        }
                        case OP_BSREM_I: {
                            if (res == 0) {
                        ...
                        case OP_BUREM_I: {
                            res = res == 0
                                ? f.m_res & MASK(SIZE(APP(f.m_curr)))
                                : (f.m_res % res) & MASK(SIZE(APP(f.m_curr)));
                            break;
                        }
#if 0
...
#define EVAL_ARG(args, i)   Z3_custom_eval_internal(c, of_ast(args[i]), data, symbols_sizes, data_size)
    ...
    uint64_t Z3_API Z3_custom_eval_depth(Z3_context c, Z3_ast _expr, uint64_t* data, uint8_t* symbols_sizes, size_t data_size, uint32_t* depth) {
        ...
        *depth = 0;
        ...
        size_t   i   = 0;
        do {
            uint64_t arg1 = Z3_custom_eval_internal_iter(
                c, of_ast(args[i]), data, symbols_sizes, data_size);
            if (!arg1) {
                res = 0;
                break;
            } else
